...
from app.bot.helper import create_invoice, get_prices, get_sale_percent
...
async def get_db_user(
    ...
    try:
        data = safe_parse_webapp_init_data(token=bot.token, init_data=auth)
    except ValueError:
...
def prices(user: TGUserDep):
    ...
    """
    is_friend = True if user.invited_by else False
    durations = [7, 14, 30, 60, 90, 180, 365]

    def make(c: Currency, duration: int):
        price = get_prices(c, duration, is_friend)

        return Price(
            price=sum(l.amount for l in price),
            duration=duration,
            discount_percent=get_sale_percent(duration, is_friend),
        )

    return {
        Currency.RUB: [make(Currency.RUB, d) for d in durations],
        Currency.XTR: [make(Currency.XTR, d) for d in durations],
    }
...
async def generate_invoice(
    duration: Annotated[
        int, Form(ge=1, le=365, description="Продолжительность в днях")
    ],
    user: TGUserDep,
    db: DBDep,
    currency: Annotated[Currency, Form(description="Валюта")] = Currency.RUB,
    is_subscription: Annotated[
        bool, Form(description="Ежемесячная оплата в XTR")
    ] = False,
    is_link: Annotated[
        bool, Form(description="Вернуть ссылку или отправить в чате")
    ] = True,
):
    """
